  ...
  <HeadFoot class="pg_fixpassword" :Title="title">
    <template #content>
      ...
      <van-cell-group class="newData">
        <van-cell>
          <img slot="icon" src="@/assets/img/login/pg_login_phone.png" alt class="num_img" />
          <input type="password" placeholder="新密码" class="num" v-model="newPW" @blur="validPay" />
        </van-cell>
        <van-cell>
          <img slot="icon" src="@/assets/img/login/pg_login_phone.png" alt class="num_img" />
...
import { changePayPassword } from "@/api/center.js";
    ...
    submit() {
      if (notNull(this.msn)) {
        if (this.validpw() && notNull(this.newPW)) {
          let _data = {
            vcode: this.msn,
            payPassword: this.newPW
          };
          changePayPassword(_data).then(({ data }) => {
